a) ¿Qué diferencia hay entre Scrum y Kanban? ¿Cuál te parece más útil para un proyecto individual y por qué?

Ambas son metodologías ágiles pero se diferencian en la forma en la que organizan el trabajo.

Scrum determina roles dentro del equipo y divide el proyecto en 4 eventos con una duración y tarea específicas, incluyendo entregas periódicas y feedback constante.

Kanban organiza las tareas en un tablero con columnas: Backlog, Por hacer, En curso, En revisión, Terminado, permitiendo reflejar el avance en las tareas. El flujo de las tareas es continuo, no hay sprints ni roles.

Para un proyecto individual, Kanban podría ser de mayor utilidad ya que no requiere de roles específicos, permite determinar prioridades así como avanzar y registrar dichos avances en base al ritmo y neccesidades de cada uno

b) ¿Qué es un MVP (Producto Mínimo Viable) y cómo lo aplicarías si te piden hacer un sitio web para un negocio local?

Es una versión simple de un producto que incluye las funcionalidades básicas y de valor para que funcione y pueda ser probado. 
En el caso de tener que desarrollar un sitio web lanzaría una página simple con información importante del negocio local que incluya servicios, ubicación, horarios y contacto.
Es decir, las funciones avanzadas como formulario, catálogo, sistema de compras online no se incluirían en esta primera versión (MVP).


c) ¿Cómo te ayudó el tablero Kanban a organizar las tareas de este TP? Mencioná un ejemplo concreto.

Me permite priorizar, organizar y dividir las tareas para visualizar aquellas que están pendientes, en proceso y las terminadas para llevar un seguimiento y evitar omitir alguna.
Ejemplo: Dividir las tareas del trabajo práctico, 1)formulario de contacto, 2)estilos css, etc.
